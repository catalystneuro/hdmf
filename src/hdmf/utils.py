...
__allowed_enum_types = (
    __supported_bool_types
    + __supported_uint_types
    + __supported_int_types
    + __supported_float_types
    + (str,)
)
...
def __check_enum(argval, arg):
    ...
    if argval not in arg["enum"]:
        return "forbidden value for '{}' (got {}, expected {})".format(
            arg["name"], __fmt_str_quotes(argval), arg["enum"]
        )
...
def __parse_args(
    ...
    extras = (
        dict()
    )  # has to be initialized to empty here, to avoid spurious errors reported upon early raises
    ...
    try:
        ...
        if (
            allow_extra
        ):  # extra keyword arguments are allowed so do not consider them when checking number of args
            if len(args) > len(validator):
                raise TypeError(
                    "Expected at most %d arguments %r, got %d positional"
                    % (len(validator), names, len(args))
                )
                ...
                if enforce_type:
                    if not __type_okay(argval, arg["type"]):
                        if argval is None:
                            ...
                            type_errors.append(
                                "None is not allowed for '%s' (expected '%s', not None)"
                                % fmt_val
                            )
                        else:
                            fmt_val = (
                                argname,
                                type(argval).__name__,
                                __format_type(arg["type"]),
                            )
                            type_errors.append(
                                "incorrect type for '%s' (got '%s', expected '%s')"
                                % fmt_val
                            )
                if enforce_shape and "shape" in arg:
                    ...
                    while valshape is None:
                        ...
                        if not hasattr(argval, argname):
                            ...
                            value_errors.append(
                                "cannot check shape of object '%s' for argument '%s' "
                        ...
                        valshape = get_data_shape(argval)
                    if valshape is not None and not __shape_okay_multi(
                        argval, arg["shape"]
                    ):
                        fmt_val = (argname, valshape, arg["shape"])
                        value_errors.append(
                            "incorrect shape for '%s' (got '%s', expected '%s')"
                            % fmt_val
                        )
                if "enum" in arg:
            ...
            if enforce_shape and "shape" in arg and argval is not None:
                ...
                while valshape is None:
                    ...
                    if not hasattr(argval, argname):
                        fmt_val = (argval, argname, arg["shape"])
                        value_errors.append(
                            "cannot check shape of object '%s' for argument '%s' (expected shape '%s')"
                            % fmt_val
                        )
                        break
                    # unpack, e.g. if TimeSeries is passed for arg 'data', then TimeSeries.data is checked
                ...
                if valshape is not None and not __shape_okay_multi(
                    ...
                    fmt_val = (argname, valshape, arg["shape"])
                    value_errors.append(
                        "incorrect shape for '%s' (got '%s', expected '%s')" % fmt_val
                    )
    ...
    if func_docval:
        if args:
            ...
            try:
                ...
                raise ValueError(
                    "Function %s does not have docval argument %s"
                    % (func.__name__, str(ke))
                )
...
def __resolve_type(t):
    ...
    if isinstance(t, str):
        ...
        msg = "argtype must be a type, a str, a list, a tuple, or None - got %s" % type(
            t
        )
    ...
    def dec(func):
        ...
        for a in validator:
            ...
            if unsupported_terms:
                raise Exception(
                    "docval for {}: keys {} are not supported by docval".format(
                        a["name"], sorted(unsupported_terms)
                    )
                )
            ...
            if "enum" in a:
                ...
                if not isinstance(a["enum"], (list, tuple)):
                    msg = (
                        "docval for %s: enum value must be a list or tuple (received %s)"
                        % (a["name"], type(a["enum"]))
                    )
                    raise Exception(msg)
                # check that arg type is compatible with enum
                if not __check_enum_argtype(a["type"]):
                    msg = "docval for {}: enum checking cannot be used with arg type {}".format(
                        a["name"], a["type"]
                    )
                    raise Exception(msg)
                ...
                if any([not __type_okay(x, a["type"]) for x in a["enum"]]):
                    msg = (
                        "docval for {}: enum values are of types not allowed by arg type (got {}, "
                        "expected {})".format(
                            a["name"], [type(x) for x in a["enum"]], a["type"]
                        )
                    )
                    raise Exception(msg)
            if "default" in a:
        ...
        def _check_args(args, kwargs):
            ...
            for error_type, ExceptionType in (
        ...
        docstring = __googledoc(
            func, _docval[__docval_args_loc], returns=returns, rtype=_rtype
        )
        docval_idx = {
            a["name"]: a for a in _docval[__docval_args_loc]
        }  # cache a name-indexed dictionary of args
...
def __builddoc(
    func, validator, docstring_fmt, arg_fmt, ret_fmt=None, returns=None, rtype=None
):
...
class LabelledDict(dict):
    ...
    @docval(
        {"name": "label", "type": str, "doc": "the label on this dictionary"},
        {
            "name": "key_attr",
            "type": str,
            "doc": "the attribute name to use as the key",
            "default": "name",
        },
        {
            "name": "add_callable",
        ...
        {
            "name": "remove_callable",
            "type": types.FunctionType,
            "doc": (
                "function to call on an element after removing it from this dict using the pop, popitem, clear, "
    ...
    def __setitem__(self, key, value):
        ...
        if key in self:
            raise TypeError(
                "Key '%s' is already in this dict. Cannot reset items in a %s."
                % (key, self.__class__.__name__)
            )
        self.__check_value(value)
        if key != getattr(value, self.key_attr):
            raise KeyError(
                "Key '%s' must equal attribute '%s' of '%s'."
                % (key, self.key_attr, value)
            )
    ...
    def __check_value(self, value):
        if not hasattr(value, self.key_attr):
            raise ValueError(
                "Cannot set value '%s' in %s. Value must have attribute '%s'."
